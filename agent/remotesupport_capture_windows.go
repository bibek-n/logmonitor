...
// ffmpegPath looks in PATH first, then next to this running executable itself - ffmpeg has
// no other footprint in this app, so the simplest deployable option is dropping ffmpeg.exe
// alongside the agent binary rather than requiring a separate system-wide install step.
func ffmpegPath() (string, error) {
	...
	return "", fmt.Errorf("ffmpeg.exe not found in PATH or next to agent.exe")
}

...
import { parsePagination } from "@/lib/laravelSecurityShared";
...
export async function GET(req: NextRequest) {
  ...
  const countResult = await countRequest.query<{ Total: number }>(`
    SELECT COUNT(*) AS Total FROM LaravelSecurityIssues i JOIN LaravelSecurityProjects p ON p.Id = i.ProjectId WHERE ${whereClause}
  `);

...
import { upsertProjectSchema } from "@/lib/laravelSecurityShared";
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await db.request().input("id", sql.Int, id).query(`
    SELECT p.Id, p.Name, p.Description, p.RepositoryUrl, p.SourcePath, p.DefaultBranch, p.LaravelVersion, p.ScanConfig, p.Status,
  ...
  const scanHistory = await db.request().input("projectId", sql.Int, id).query(`
    ...
    FROM LaravelSecurityScans WHERE ProjectId = @projectId ORDER BY CreatedAt DESC
  `);
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const ls = await requireLaravelSecurityPermission("ls_project_update");
  if (!isLsSession(ls)) return ls;
  ...
  const db = await getDb();
  const existing = await db.request().input("id", sql.Int, id).query<{ Id: number }>("SELECT Id FROM LaravelSecurityProjects WHERE Id = @id AND DeletedAt IS NULL");
  ...
  // Same deferred-resync reasoning as codeQuality's projects/[id]/route.ts PUT: repointing a
  // project doesn't resync immediately, the next scan syncs before it runs.
  ...
  await request.query(`UPDATE LaravelSecurityProjects SET ${setClauses.join(", ")} WHERE Id = @id`);

  await logAdminAction({ admin: ls, section: "laravel-security", action: "update_project", details: `Project #${id}`, req });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.request().input("id", sql.Int, id).query("UPDATE LaravelSecurityProjects SET DeletedAt = SYSUTCDATETIME() WHERE Id = @id");
  await logAdminAction({ admin: ls, section: "laravel-security", action: "delete_project", details: `Project #${id}`, req });

...
import { runSeoScan } from "@/lib/seoScanner/runScan";
import { saveSeoScan } from "@/lib/seoScanner/persist";
...
export async function POST(req: NextRequest) {
  ...
  let report;
  try {
    report = await runSeoScan(url);
  } catch (err) {
    return NextResponse.json({ ok: false, error: err instanceof Error ? err.message : "Scan failed." });
  }

  const scanId = await saveSeoScan(report, websiteId, admin);
  await logAdminAction({ admin, section: "seo-scanner", action: "scan", details: url, req });

...
import { updateApiMonitorSchema } from "@/lib/websiteApiMonitoring/schema";
import { loadApiMonitorById } from "@/lib/websiteApiMonitoring/repository";
import { encryptApiAuthConfig, maskApiAuthConfig, mergeApiAuthConfigSecrets } from "@/lib/websiteApiMonitoring/apiCredentials";
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ ok: true, data: { ...monitor, config: { ...monitor.config, authConfig: maskApiAuthConfig(monitor.config.authConfig) } } });
}

export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const mon = await requireMonitoringPermission("mon_api_edit");
  if (!isMonitoringSession(mon)) return mon;

  const { id } = await params;
  const monitorId = Number(id);
  const existing = await loadApiMonitorById(monitorId);
  if (!existing) return NextResponse.json({ ok: false, error: "Monitor not found" }, { status: 404 });

  const body = await req.json().catch(() => null);
  const parsed = updateApiMonitorSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ ok: false, error: parsed.error.issues[0]?.message ?? "Invalid monitor payload" }, { status: 400 });
  }
  const p = parsed.data;

  // Any secret field the caller left blank/unchanged (still the mask placeholder from the GET
  // above) keeps the existing decrypted value instead of being overwritten with a blank - see
  // apiCredentials.ts's mergeApiAuthConfigSecrets for why.
  const mergedAuthConfig = p.authConfig !== undefined ? mergeApiAuthConfigSecrets(p.authConfig, existing.config.authConfig) : existing.config.authConfig;
  ...
  const merged = {
    ...
    headers: p.headers ?? existing.config.headers,
    queryParams: p.queryParams ?? existing.config.queryParams,
    requestBody: p.requestBody !== undefined ? p.requestBody : existing.config.requestBody,
    requestBodyContentType: p.requestBodyContentType !== undefined ? p.requestBodyContentType : existing.config.requestBodyContentType,
    authType: p.authType ?? existing.config.authType,
    ...
    assertions: p.assertions ?? existing.config.assertions,
  ...
  await db
    ...
    .input("headersJson", sql.NVarChar(sql.MAX), JSON.stringify(merged.headers))
    .input("queryParamsJson", sql.NVarChar(sql.MAX), JSON.stringify(merged.queryParams))
    .input("requestBody", sql.NVarChar(sql.MAX), merged.requestBody)
    .input("requestBodyContentType", sql.VarChar, merged.requestBodyContentType)
    .input("authType", sql.VarChar, merged.authType)
    .input("authConfigEncrypted", sql.NVarChar(sql.MAX), encryptApiAuthConfig(merged.authConfig))
    ...
    .input("assertionsJson", sql.NVarChar(sql.MAX), JSON.stringify(merged.assertions))
    ...
    .query(`
      UPDATE ApiMonitorConfigs SET Url=@url, HttpMethod=@httpMethod, HeadersJson=@headersJson, QueryParamsJson=@queryParamsJson,
        RequestBody=@requestBody, RequestBodyContentType=@requestBodyContentType, AuthType=@authType,
        AuthConfigEncrypted=@authConfigEncrypted, ExpectedStatusCode=@expectedStatusCode, FollowRedirects=@followRedirects,
        MaxRedirects=@maxRedirects, SslVerify=@sslVerify, AssertionsJson=@assertionsJson,
        ResponseTimeWarningMs=@responseTimeWarningMs, ResponseTimeCriticalMs=@responseTimeCriticalMs, AlertEmail=@alertEmail
      WHERE MonitorId=@monitorId
    `);

  await logAdminAction({ admin: mon, section: "monitoring", action: "api_monitor_update", details: merged.name, req });

  return NextResponse.json({ ok: true });
}

export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const mon = await requireMonitoringPermission("mon_api_delete");
  if (!isMonitoringSession(mon)) return mon;
  ...
  await logAdminAction({ admin: mon, section: "monitoring", action: "api_monitor_delete", details: existing.name, req });

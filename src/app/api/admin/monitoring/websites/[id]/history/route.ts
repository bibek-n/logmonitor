...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await db
    ...
    // CheckedAt selected as raw DATETIME2 (never CONVERT(...,126) to a Z-less string) so
    // JSON.stringify emits a real ...Z ISO string - see the identical fix and comment in
    // websiteApiMonitoring/repository.ts's JOIN_SELECT.
    .query(`
      SELECT Id, CheckedAt, Success, HttpStatusCode, DnsMs, TcpMs, TlsMs, TtfbMs,
        TotalMs, ResponseSizeBytes, RedirectCount, FinalUrl, ContentCheckResultJson, ErrorCode, ErrorMessage
      FROM MonitorResults
      WHERE MonitorId = @id

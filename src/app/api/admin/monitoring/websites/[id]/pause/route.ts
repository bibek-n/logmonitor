...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.request().input("id", sql.Int, monitor.id).query("UPDATE Monitors SET Status = 'Paused' WHERE Id = @id");

  await logAdminAction({ admin: mon, section: "monitoring", action: "website_monitor_pause", details: monitor.name, req });

...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.request().input("id", sql.Int, policy.id).query("UPDATE MailBlockingPolicies SET Enabled = 1, UpdatedAt = SYSUTCDATETIME() WHERE Id = @id");

  await logAdminAction({ admin: mail, section: "mail-security", action: "policy_enable", details: policy.name, req });

...
import { updateMaintenanceLogSchema } from "@/lib/itAssetLogsheet/schema";
import { getMaintenanceLogById, softDeleteMaintenanceLog, updateMaintenanceLog } from "@/lib/itAssetLogsheet/repository";
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string; logId: string }> }) {
  ...
  const existing = await getMaintenanceLogById(Number(logId));
  ...
  const result = await updateMaintenanceLog(Number(logId), parsed.data, { userId: ita.userId, username: ita.username });
  if (result && "error" in result) {
    return NextResponse.json({ ok: false, error: result.error }, { status: 400 });
  }
  await logAdminAction({ admin: ita, section: "it-asset-logsheet", action: "maintenance_update", details: logId, req });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string; logId: string }> }) {
  ...
  await softDeleteMaintenanceLog(Number(logId), ita.userId);
  await logAdminAction({ admin: ita, section: "it-asset-logsheet", action: "maintenance_delete", details: logId, req });

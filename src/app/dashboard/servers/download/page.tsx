...
export default async function ServersDownloadAgentPage() {
  const admin = await getAdminSession();
  if (!admin) {
    return (
      <div>
        <h1 style={{ fontSize: "1.4rem" }}>Download Agent</h1>
        <p style={{ color: "var(--danger)" }}>Only admins can download the agent.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <h1 style={{ fontSize: "1.4rem", marginBottom: "0.25rem" }}>Download Agent (Servers)</h1>
      <p style={{ color: "var(--ink-muted)", fontSize: "0.85rem", marginTop: 0, marginBottom: "0.5rem" }}>
        ...
        This is the same agent used for workstations, extended with deep hardware discovery (CPU/RAM/motherboard/BIOS/
        serial/disks/network interfaces) and Apache/PHP/MySQL/system log shipping. Register the server first from the{" "}
        <Link href="/dashboard/servers/add" style={{ color: "var(--primary)" }}>
          Add Server
        </Link>{" "}
        page to get a matching install command with a pre-linked enrollment token.
      </p>

...
export default async function EditNetworkDiagramDesignPage({ params }: { params: Promise<{ id: string }> }) {
  const admin = await getAdminSession();
  if (!admin) {
    return (
      <div>
        <h1 style={{ fontSize: "1.4rem" }}>Edit Network Diagram</h1>
        <p style={{ color: "var(--danger)" }}>Only admins can edit network diagrams.</p>
      </div>
    );
  }

...
// Mirrors codeQualityShared.ts's upsertProjectSchema/createProjectSchema split exactly - see
// that file's own comment for why sourcePath is optional here and enforced by
// createProjectSchema's superRefine instead. repoSourceFields/hasRepoSource are the same
// shared-connections plumbing every module uses (see repoConnectionsShared.ts).
...
export const issueCategorySchema = z.enum(["AppDebug", "AppKey", "DotEnv", "Csrf", "MassAssignment", "Validation", "Sanitization", "StorageLinks", "Queue"]);

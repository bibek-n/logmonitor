...
// Website & API Monitoring module's permission gate — follows the same PERMISSION_KEYS/
// RolePermissions mechanism QA Testing activated first (see requireQaPermission.ts).
// Deliberately scoped to /api/admin/monitoring/** only.
//
...
async function resolveMonitoringSession(permissionKey: string): Promise<MonitoringSession | null> {
...
// For Server Component pages: one query resolving every mon_* grant for the caller's role at
...
// requireMonitoringPermission(), so hiding a button here changes nothing about enforcement.
export async function getMonitoringAccess(): Promise<{ mon: MonitoringSession | null; can: Record<string, boolean> }> {
  const base = await resolveBaseSession();
  if (!base) return { mon: null, can: {} };

  if (base.role === "Admin") {
    return { mon: base, can: Object.fromEntries(MONITORING_PERMISSION_KEYS.map((k) => [k, true])) };
  }
  ...
  const can: Record<string, boolean> = Object.fromEntries(MONITORING_PERMISSION_KEYS.map((k) => [k, false]));
  for (const grant of grants.recordset) {
    if (grant.Allowed && grant.PermissionKey in can) can[grant.PermissionKey] = true;
  }

  return { mon: can.mon_view ? base : null, can };
}

export async function requireMonitoringPermission(permissionKey: string): Promise<MonitoringSession | NextResponse> {
  const mon = await resolveMonitoringSession(permissionKey);
  if (!mon) {
...
export function isMonitoringSession(value: MonitoringSession | NextResponse): value is MonitoringSession {
...
export async function getMonitoringSession(permissionKey: string): Promise<MonitoringSession | null> {
  return resolveMonitoringSession(permissionKey);
}

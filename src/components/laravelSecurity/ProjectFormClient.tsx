...
const ADD_PROJECT_RETURN_TO = "/dashboard/laravel-security/projects/new";

// Note: unlike Code Quality's ProjectFormClient, there is no "Programming Language" field here
// - Laravel Security projects track LaravelVersion instead, which is detected automatically
// from composer.json/artisan during a scan (see runScan.ts) and is never user-editable. It's
// shown as a read-only badge on ProjectDetailClient once a scan has run, not in this form.
export function ProjectFormClient({ projectId, initial }: { projectId?: number; initial?: Partial<ProjectFormValues> }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const res = await fetch(isEdit ? `/api/admin/laravel-security/projects/${projectId}` : "/api/admin/laravel-security/projects", {
      ...
      router.push(isEdit ? `/dashboard/laravel-security/projects/${projectId}` : `/dashboard/laravel-security/projects/${data.data.id}`);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col" style={{ gap: "1rem", maxWidth: 640 }}>
            ...
            {sourceType === "local" && (
              <div>
                ...
                <div className="flex gap-2">
                  <input style={{ ...fieldStyle, fontFamily: "monospace" }} value={form.sourcePath} onChange={(e) => set("sourcePath", e.target.value)} placeholder="D:\WWWROOT\MyLaravelApp" />
      ...
      <FolderBrowseModal
        ...
        apiPath="/api/admin/laravel-security/browse-folders"
      />
    </div>
  );
}

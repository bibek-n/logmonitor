...
export function ProjectsListClient({ can }: { can: Record<string, boolean> }) {
  ...
  const { rows, pagination, loading, page, setPage, sortBy, sortDir, onSortChange, reload } = useLaravelSecurityList<ProjectRow>(
    "/api/admin/laravel-security/projects",
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/admin/laravel-security/projects/${deleteTarget.Id}`, { method: "DELETE" });
  ...
  const columns: LsTableColumn<ProjectRow>[] = [
    { key: "Name", label: "Project Name", sortable: true, render: (r) => <Link href={`/dashboard/laravel-security/projects/${r.Id}`} style={{ color: "var(--primary)", textDecoration: "none", fontWeight: 600 }}>{r.Name}</Link> },
    { key: "SourcePath", label: "Source Path", render: (r) => <span style={{ fontFamily: "monospace", fontSize: "0.78rem" }}>{r.SourcePath}</span> },
    { key: "LaravelVersion", label: "Laravel Version", render: (r) => r.LaravelVersion ?? "—" },
    ...
    { key: "SecurityScore", label: "Security Score", render: (r) => <SecurityScoreBadge score={r.SecurityScore} /> },
  ...
  return (
    <div className="flex flex-col" style={{ gap: "1rem" }}>
      <div className="flex items-center justify-between">
        ...
        {can.ls_project_create && (
          <Link href="/dashboard/laravel-security/projects/new" style={{ padding: "0.55rem 1rem", borderRadius: 8, background: "var(--primary)", color: "#fff", textDecoration: "none", fontSize: "0.85rem", fontWeight: 600 }}>
            Add Project
          </Link>
        )}
      </div>
      ...
      <LaravelSecurityTable
        ...
        rowActions={(r) => (
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="sm" onClick={() => router.push(`/dashboard/laravel-security/projects/${r.Id}`)} title="View">
              <Eye size={14} />
            </Button>
            {can.ls_project_update && (
              <Button variant="ghost" size="sm" onClick={() => router.push(`/dashboard/laravel-security/projects/${r.Id}/edit`)} title="Edit">
                <Pencil size={14} />
              </Button>
            )}
            {can.ls_scan_start && (
            ...
            <Button variant="ghost" size="sm" onClick={() => router.push(`/dashboard/laravel-security/scans?projectId=${r.Id}`)} title="View Scan History">
              <History size={14} />
            </Button>
            {can.ls_project_delete && (
      ...
      <ConfirmDialog
        ...
        message={`This removes "${deleteTarget?.Name}" from Laravel Security. Its scan history is kept for audit purposes.`}
        confirmLabel="Delete"
        tone="danger"
        loading={deleting}
      />

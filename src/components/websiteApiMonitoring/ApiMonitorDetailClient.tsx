...
type TabKey = "overview" | "history" | "incidents" | "config";
...
interface MonitorRow {
  ...
  config: {
    ...
    sslVerify: boolean;
    assertions: { path: string; operator: string; expectedValue: string | null }[];
...
export function ApiMonitorDetailClient({ monitorId }: { monitorId: number }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/monitoring/api/${monitorId}`)
      .then((r) => r.json())
      .then((d) => d.ok && setMonitor(d.data));
    fetch(`/api/admin/monitoring/api/${monitorId}/metrics?days=7`)
      .then((r) => r.json())
  ...
  useEffect(() => {
    if (tab === "history" && history === null) {
      fetch(`/api/admin/monitoring/api/${monitorId}/history`)
        .then((r) => r.json())
  ...
  }, [tab, monitorId, history, incidents]);
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "0.25rem", flexWrap: "wrap", gap: "0.5rem" }}>
        ...
        <Link href={`/dashboard/monitoring/api/${monitorId}/edit`} style={{ color: "var(--series-1)", fontSize: "0.85rem" }}>
          Edit monitor
        </Link>
      </div>
      <p style={{ color: "var(--ink-muted)", fontSize: "0.85rem", marginTop: 0, fontFamily: "monospace" }}>
        {monitor.config.httpMethod} {monitor.config.url}
      </p>
        ...
        <div className="dash-panel">
          <table style={{ fontSize: "0.85rem", borderCollapse: "collapse" }}>
            <tbody>
              ...
              <tr>
                <td style={{ padding: "0.3rem 1rem 0.3rem 0", color: "var(--ink-muted)" }}>Auth Type</td>
                <td style={{ padding: "0.3rem 0" }}>{monitor.config.authType}</td>
              </tr>
              <tr>
                <td style={{ padding: "0.3rem 1rem 0.3rem 0", color: "var(--ink-muted)" }}>Headers</td>
                <td style={{ padding: "0.3rem 0" }}>{monitor.config.headers.length}</td>
              </tr>
              <tr>
                <td style={{ padding: "0.3rem 1rem 0.3rem 0", color: "var(--ink-muted)" }}>Assertions</td>
                <td style={{ padding: "0.3rem 0" }}>{monitor.config.assertions.length}</td>
              </tr>
              <tr>

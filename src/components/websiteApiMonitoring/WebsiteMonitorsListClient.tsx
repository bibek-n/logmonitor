...
import { ImportWebsitesModal } from "./ImportWebsitesModal";
...
interface MonitorRow {
  ...
  nextCheckAt: string;
  config: { url: string; expectedStatusCode: number };
}
...
function WebsiteMonitorsInner() {
  ...
  async function callAction(id: number, action: "pause" | "resume" | "duplicate", label: string) {
    const res = await fetch(`/api/admin/monitoring/websites/${id}/${action}`, { method: "POST" });
    const data = await res.json();
    if (!res.ok || !data.ok) {
  ...
  async function remove(id: number, name: string) {
    const res = await fetch(`/api/admin/monitoring/websites/${id}`, { method: "DELETE" });
    const data = await res.json();
    if (!res.ok || !data.ok) {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "1rem", gap: "0.75rem", flexWrap: "wrap" }}>
        ...
        <Link href="/dashboard/monitoring/websites/new">
          <Button>New Website Monitor</Button>
        </Link>
      </div>

      <ImportWebsitesModal open={importOpen} onClose={() => setImportOpen(false)} onImported={load} />
      <SendReportModal open={reportOpen} onClose={() => setReportOpen(false)} />

      {monitors === null ? (
        <p style={{ color: "var(--ink-muted)" }}>Loading...</p>
      ) : filtered.length === 0 ? (
        <p style={{ color: "var(--ink-muted)" }}>No website monitors yet.</p>
      ) : (
              ...
              <tbody>
                {filtered.map((m, index) => (
                  <tr key={m.id} style={{ borderBottom: "1px solid var(--grid)" }}>
                    ...
                    <td style={{ padding: "0.4rem" }}>
                      <Link href={`/dashboard/monitoring/websites/${m.id}`} style={{ color: "var(--series-1)" }}>
                        {m.name}
                      </Link>
                    </td>
                    ...
                    <td style={{ padding: "0.4rem", whiteSpace: "nowrap" }}>
                      <div style={{ display: "flex", gap: "0.3rem" }}>
                        <Link href={`/dashboard/monitoring/websites/${m.id}/edit`}>
...
export function WebsiteMonitorsListClient() {
  return (
    <ToastProvider>
      <WebsiteMonitorsInner />
    </ToastProvider>
  );
}

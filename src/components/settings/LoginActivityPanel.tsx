...
export function LoginActivityPanel({ rows }: { rows: LoginActivityRow[] }) {
  return (
    <Card className="flex flex-col gap-3" id="field-login-activity">
      <h3 style={{ fontSize: "0.95rem", margin: 0, color: "var(--ink)" }}>Login Activity</h3>
      <div style={{ overflowX: "auto", maxHeight: 320, overflowY: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8rem" }}>
          <thead>
            <tr style={{ textAlign: "left", borderBottom: "1px solid var(--border)" }}>
              {["User", "IP Address", "Result", "Details", "When"].map((h) => (
            ...
            {rows.map((r) => (
              <tr key={r.Id} style={{ borderBottom: "1px solid var(--border)" }}>
                ...
                <td style={{ padding: "0.4rem 0.6rem" }}>
                  <Badge tone={r.Success ? "success" : "danger"}>{r.Success ? "Success" : "Failed"}</Badge>
                </td>

...
import { MAINTENANCE_STATUS_TONE, humanize } from "./statusTones";
...
const STATUSES = ["Planned", "Scheduled", "InProgress", "Completed", "Failed", "Cancelled", "Deferred"];
...
export function MaintenanceLogClient() {
  ...
  const load = useCallback(async () => {
    ...
    const res = await fetch(`/api/admin/it-asset-logsheet/maintenance?${params.toString()}`);
    ...
    <div>
      ...
      <div className="dash-panel">
        {rows === null ? (
          ...
          <p style={{ color: "var(--ink-muted)" }}>No maintenance records found.</p>
        ) : (
          <>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" }}>
                <thead>
                  <tr style={{ textAlign: "left", borderBottom: "1px solid var(--border)", color: "var(--ink-muted)" }}>
                    ...
                    <th style={{ padding: "0.4rem" }}>Scheduled</th>
                    <th style={{ padding: "0.4rem" }}>Performed By</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.id} style={{ borderBottom: "1px solid var(--grid)" }}>
                      ...
                      <td style={{ padding: "0.4rem" }}>{r.activityTitle}</td>
                      <td style={{ padding: "0.4rem" }}>{humanize(r.activityType)}</td>
                      <td style={{ padding: "0.4rem" }}><Badge tone={MAINTENANCE_STATUS_TONE[r.status] ?? "neutral"}>{humanize(r.status)}</Badge></td>
                      <td style={{ padding: "0.4rem", color: "var(--ink-muted)" }}>{r.scheduledDate ?? "—"}</td>
                      <td style={{ padding: "0.4rem", color: "var(--ink-muted)" }}>{r.performedBy ?? "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

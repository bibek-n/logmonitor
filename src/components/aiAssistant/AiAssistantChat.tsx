...
const EXAMPLE_QUESTIONS = [
  "Which server has the highest CPU usage today?",
  "Show devices that went offline this week.",
  "Why is internet bandwidth high?",
  "Which SSL certificates expire this month?",
];
...
function AiAssistantChatInner() {
  ...
  async function ask(question: string) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/admin/ai-assistant/ask", {
        method: "POST",
  ...
  return (
    <Card style={{ padding: 0, display: "flex", flexDirection: "column", height: "70vh" }}>
      <div ref={scrollRef} className="flex flex-col gap-3" style={{ flex: 1, overflowY: "auto", padding: "1rem" }}>
        ...
        {loading && <p style={{ fontSize: "0.8rem", color: "var(--ink-muted)" }}>Thinking...</p>}
      </div>

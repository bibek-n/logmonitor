...
export function IssuesListClient({ can, projects }: { can: Record<string, boolean>; projects: { Id: number; Name: string }[] }) {
  ...
  const { rows, pagination, loading, page, setPage, sortBy, sortDir, onSortChange, reload } = useCodeQualityList<IssueRow>("/api/admin/code-quality/issues", {
    ...
    <div className="flex flex-col" style={{ gap: "1rem" }}>
      ...
      <div className="flex flex-col gap-2">
        <input
          ...
          placeholder="Search file, function, variable, rule, or description…"
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <select value={category} onChange={(e) => resetToFirstPage(setCategory)(e.target.value)} style={fieldStyle}>
            ...
            {["Complexity", "Duplication", "DeadCode", "UnusedVariable", "UnusedFunction", "CodingStandard"].map((c) => (
          ...
          <input value={language} onChange={(e) => resetToFirstPage(setLanguage)(e.target.value)} placeholder="Language" style={{ ...fieldStyle, width: 120 }} />
      ...
      {openIssueId !== null && (
        <IssueDetailsDrawer issueId={openIssueId} onClose={() => setOpenIssueId(null)} canUpdate={can.cq_issue_update} onUpdated={reload} />
      )}
    </div>
  );
}

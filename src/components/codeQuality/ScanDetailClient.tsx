...
import { QualityScoreBadge, ScanStatusBadge, SeverityBadge } from "./badges";
...
const SECTIONS: { category: string; label: string; description: string }[] = [
  { category: "Complexity", label: "Cyclomatic Complexity", description: "Functions and methods above the configured complexity threshold." },
  { category: "Duplication", label: "Code Duplication", description: "Blocks of code duplicated elsewhere in the project." },
  { category: "DeadCode", label: "Dead Code", description: "Code that can never execute, or exports never used anywhere." },
  { category: "UnusedVariable", label: "Unused Variables", description: "Declared variables and parameters never read." },
  { category: "UnusedFunction", label: "Unused Functions", description: "Declared functions never called or imported." },
  { category: "CodingStandard", label: "Coding Standards", description: "Style and convention violations." },
];
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const [logRes, scanRes] = await Promise.all([
        fetch(`/api/admin/code-quality/scans/${scanId}/log?since=${sinceRef.current}`).then((r) => r.json()),
        fetch(`/api/admin/code-quality/scans/${scanId}`).then((r) => r.json()),
      ]);
      if (cancelled) return;
...
export function ScanDetailClient({ scan: initialScan, issueCounts: initialIssueCounts, can }: { scan: Scan; issueCounts: IssueCount[]; can: Record<string, boolean> }) {
  ...
  const [activeCategory, setActiveCategory] = useState<string>("Complexity");
  const [openIssueId, setOpenIssueId] = useState<number | null>(null);
  const inFlight = ["Pending", "Queued", "Running"].includes(scan.Status);

  const { rows, pagination, loading, page, setPage } = useCodeQualityList<IssueRow>("/api/admin/code-quality/issues", {
    scanId: String(scan.Id),
    category: activeCategory,
  });

  async function refreshIssueCounts() {
    const res = await fetch(`/api/admin/code-quality/scans/${scan.Id}`).then((r) => r.json());
  ...
  const columns: CqTableColumn<IssueRow>[] = [
    ...
    <div className="flex flex-col" style={{ gap: "1rem" }}>
      <div className="flex items-center justify-between flex-wrap" style={{ gap: "0.5rem" }}>
        ...
        <div className="flex items-center gap-2">
          ...
          {can.cq_export && (
            <Button variant="secondary" onClick={() => window.open(`/api/admin/code-quality/scans/${scan.Id}/export?format=csv`, "_blank")}>
      ...
      <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))" }}>
        ...
        <Card style={{ padding: "0.85rem" }}>
          <div style={{ fontSize: "0.72rem", color: "var(--ink-muted)" }}>Lines of Code</div>
          <div>{scan.LinesOfCode.toLocaleString()}</div>
        </Card>
        <Card style={{ padding: "0.85rem" }}>
          <div style={{ fontSize: "0.72rem", color: "var(--ink-muted)" }}>Quality Score</div>
          <QualityScoreBadge score={scan.QualityScore} />
        </Card>
        <Card style={{ padding: "0.85rem" }}>
      ...
      {openIssueId !== null && <IssueDetailsDrawer issueId={openIssueId} onClose={() => setOpenIssueId(null)} canUpdate={can.cq_issue_update} onUpdated={refreshIssueCounts} />}
    </div>
  );
}

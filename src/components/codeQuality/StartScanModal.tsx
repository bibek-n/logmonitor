...
export function StartScanModal({
  ...
  const [complexityThreshold, setComplexityThreshold] = useState("");
  const [duplicationThreshold, setDuplicationThreshold] = useState("");
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/code-quality/rules")
      .then((r) => r.json())
      .then((data) => {
        if (!data.ok) return;
        setRules(data.data);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/admin/code-quality/scans", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          complexityThreshold: complexityThreshold ? Number(complexityThreshold) : undefined,
          duplicationThreshold: duplicationThreshold ? Number(duplicationThreshold) : undefined,
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button type="submit" form="start-scan-form" disabled={submitting}>
            {submitting ? "Starting…" : "Start Scan"}
          </Button>
        </>
      }
    >
      <form id="start-scan-form" onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <div>
          ...
          <input style={fieldStyle} value={includedDirectories} onChange={(e) => setIncludedDirectories(e.target.value)} placeholder="src, lib" />
        </div>
        <div>
          <label style={labelStyle}>Additional Excluded Directories (comma-separated)</label>
          <input style={fieldStyle} value={excludedDirectories} onChange={(e) => setExcludedDirectories(e.target.value)} placeholder="generated, vendor" />
        </div>

        <div className="grid gap-3" style={{ gridTemplateColumns: "1fr 1fr" }}>
          <div>
            <label style={labelStyle}>Complexity Threshold (override)</label>
            <input style={fieldStyle} type="number" min={1} value={complexityThreshold} onChange={(e) => setComplexityThreshold(e.target.value)} placeholder="Default from Settings" />
          </div>
          <div>
            <label style={labelStyle}>Duplication Threshold % (override)</label>
            <input style={fieldStyle} type="number" min={0} max={100} value={duplicationThreshold} onChange={(e) => setDuplicationThreshold(e.target.value)} placeholder="Default from Settings" />
          </div>
        </div>

        {rules.length > 0 && (
          <div>
            <label style={labelStyle}>Enabled Quality Rules</label>

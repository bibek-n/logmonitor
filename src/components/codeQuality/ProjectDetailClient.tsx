    ...
    <div className="flex flex-col" style={{ gap: "1rem" }}>
      <div className="flex items-center justify-between flex-wrap" style={{ gap: "0.5rem" }}>
        ...
        <div className="flex items-center gap-2">
          {can.cq_scan_start && project.Status === "Active" && (
            <Button onClick={() => setScanModalOpen(true)}>
              <Play size={14} /> Start Scan
            </Button>
          )}
          {can.cq_project_update && (
            <Button variant="secondary" onClick={() => router.push(`/dashboard/code-quality/projects/${project.Id}/edit`)}>
              <Pencil size={14} /> Edit
            </Button>
          )}
        </div>
      ...
      <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))" }}>
        ...
        <Card style={{ padding: "0.9rem" }}>
          <div style={{ fontSize: "0.75rem", color: "var(--ink-muted)" }}>Language</div>
      ...
      <Card>
        <div className="flex items-center justify-between" style={{ marginBottom: "0.75rem" }}>
          ...
          <Link href={`/dashboard/code-quality/scans?projectId=${project.Id}`} className="flex items-center gap-1" style={{ color: "var(--primary)", fontSize: "0.8rem", textDecoration: "none" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" }}>
            <thead>
              <tr style={{ textAlign: "left", borderBottom: "1px solid var(--border)" }}>
                ...
                <th style={{ padding: "0.5rem 0.6rem", color: "var(--ink-muted)", fontWeight: 500 }}>LOC</th>
              </tr>
            </thead>
            <tbody>
              {recentScans.map((s) => (
                <tr key={s.Id} style={{ borderBottom: "1px solid var(--border)", cursor: "pointer" }} onClick={() => router.push(`/dashboard/code-quality/scans/${s.Id}`)}>
                  ...
                  <td style={{ padding: "0.5rem 0.6rem" }}>{s.LinesOfCode.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>

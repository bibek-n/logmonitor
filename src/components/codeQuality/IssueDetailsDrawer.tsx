...
export function IssueDetailsDrawer({
  ...
  useEffect(() => {
    fetch(`/api/admin/code-quality/issues/${issueId}`)
      .then((r) => r.json())
      .then((data) => {
        if (!data.ok) return;
        setIssue(data.data);
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/code-quality/issues/${issueId}`, {
        method: "PATCH",
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            <div style={labelRow}>
              ...
              <Link href={`/dashboard/code-quality/projects/${issue.ProjectId}`} style={{ color: "var(--primary)", textDecoration: "none" }}>{issue.ProjectName}</Link>
            </div>
            <div style={labelRow}>
              <span style={{ color: "var(--ink-muted)" }}>Scan</span>
              <Link href={`/dashboard/code-quality/scans/${issue.ScanId}`} style={{ color: "var(--primary)", textDecoration: "none" }}>#{issue.ScanId} ({issue.ScanBranch || "—"})</Link>
            </div>
            <div style={labelRow}>
          ...
          {issue.CodeSnippet && (
            <div>
              ...
              {/* React escapes text content automatically - safe against HTML injection from a
                  scanned file's own source text (see the module's Code Snippet Display

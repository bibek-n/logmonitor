...
export function ScansListClient({ can, initialProjectId }: { can: Record<string, boolean>; initialProjectId?: string }) {
  ...
  const { rows, pagination, loading, page, setPage, sortBy, sortDir, onSortChange, reload } = useCodeQualityList<ScanRow>(
    "/api/admin/code-quality/scans",
  ...
  async function handleCancel() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/code-quality/scans/${cancelTarget.Id}/cancel`, { method: "POST" });
  ...
  async function handleRetry(scan: ScanRow) {
    try {
      const res = await fetch(`/api/admin/code-quality/scans/${scan.Id}/retry`, { method: "POST" });
      const data = await res.json();
      if (!res.ok || !data.ok) throw new Error(data.error ?? "Failed to retry scan.");
      toast.show({ type: "success", message: "New scan started." });
      router.push(`/dashboard/code-quality/scans/${data.data.scanId}`);
    } catch (err) {
  ...
  async function handleDelete() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/code-quality/scans/${deleteTarget.Id}`, { method: "DELETE" });
      const data = await res.json();
  ...
  const columns: CqTableColumn<ScanRow>[] = [
    ...
    { key: "QualityScore", label: "Quality Score", sortable: true, render: (r) => <QualityScoreBadge score={r.QualityScore} /> },
      ...
      <CodeQualityTable
        ...
        rowActions={(r) => (
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="sm" onClick={() => router.push(`/dashboard/code-quality/scans/${r.Id}`)} title="View Results">
              <Eye size={14} />
            </Button>
            {can.cq_scan_cancel && ["Pending", "Queued", "Running"].includes(r.Status) && (
            ...
            {can.cq_export && (
              <Button variant="ghost" size="sm" onClick={() => window.open(`/api/admin/code-quality/scans/${r.Id}/export?format=csv`, "_blank")} title="Export Results">
                <Download size={14} />
              </Button>
            )}
            {can.cq_project_delete && !["Pending", "Queued", "Running"].includes(r.Status) && (
